...
function filterProduct($idStore)
{
    global $connectDb;
    $table = "product";
    $sql = " SELECT * FROM $table WHERE loai = '" . $idStore . "' ";
    $rs = mysqli_query($connectDb, $sql);
    $result = mysqli_fetch_all($rs, MYSQLI_ASSOC);
    return $result;
}

if (isset($_POST['loai']) && isset($_POST['action']) && $_POST['action'] == "filterProduct") {
    $search = filterProduct($_POST['loai']);
    if (sizeof($search) > 0) {
        $stt = 0;

...
function filterPost($idStore)
{
    global $connectDb;
    $table = "post";
    $sql = " SELECT * FROM $table WHERE chuyenMuc = '" . $idStore . "' ";
    $rs = mysqli_query($connectDb, $sql);
    $result = mysqli_fetch_all($rs, MYSQLI_ASSOC);
    return $result;
}

if (isset($_POST['chuyenMuc']) && isset($_POST['action']) && $_POST['action'] == "filterPost") {
    $search = filterPost($_POST['chuyenMuc']);
    if (sizeof($search) > 0) {
        $stt = 0;
